...
namespace _01
{
    class Config
    {
        ...
        public void LoadFromXmlFile()  //从xml读取信息
        {
            ...
            {
                //优先读取外部BlockSet.xml 文件并把信息写入blockInfo内
            ...
            else 
            {
                ...
                Stream sm = asm.GetManifestResourceStream("_01.BlockSet.xml");
        ...
        { 
            ...
            xelBlockColor.InnerText = _rectPix.ToString();
            xelSurface.AppendChild(xelBlockColor);


            root.AppendChild(xelSurface);

            doc.Save("BlockSet.xml");//保存在硬盘上
        }
